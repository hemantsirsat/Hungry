...
const Categories = ({ category, destination, api_id, api_key, to }) =>{
    const [FetchRecipe, Result, ErrorMessage] = useSearch();

    useEffect(()=>{
        FetchRecipe(category, api_id, api_key, to);
    },[]);
    if(!Result){
        return null;
    }
    ...
    return(
        <View style={styles.viewStyle}>
            <View style={styles.headingStyle}>
                ...
                <TouchableOpacity
                    onPress={()=>{
                        destination.navigate('RecipeList',{searchTerm:category,api_id,api_key,to:30})
                    }}
                >
